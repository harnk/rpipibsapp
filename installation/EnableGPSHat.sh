...
if sudo grep -i -q "splash" /boot/cmdline.txt; then
	...
	if sudo grep -i -q "plymouth.ignore-serial-consoles" /boot/cmdline.txt; then
		# you're good!
		echo "Splash screen already told to ignore serial console." | tee -a $LOGPATH/RH_install_log.txt
	else
		# add command to ignore serial console
		sudo echo "plymouth.ignore-serial-consoles" >> /boot/cmdline.txt
		echo "Updated splash screen to ignore serial console." | tee -a $LOGPATH/RH_install_log.txt
	fi
else
	echo "No splash screen is used, so no updates needed." | tee -a $LOGPATH/RH_install_log.txt
fi